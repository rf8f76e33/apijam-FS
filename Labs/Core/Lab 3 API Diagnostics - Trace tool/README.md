...
* Select the **{your_initials}**\_payment\_api that you created in an earlier lab exercise.	
...
Note: In the following snippet, the Pattern value that reads *{my_payment}* is referencing a variable in your API proxy called *my_payment*, you do not need to replace this with your initials. 

```
<?xml version="1.0" encoding="UTF-8" standalone="yes"?>
<ExtractVariables async="false" continueOnError="false" enabled="true" name="Extract-Payment">
    <DisplayName>Extract Payment</DisplayName>
    <Properties/>
    <QueryParam name="amount">
        <Pattern ignoreCase="true">{my_payment}</Pattern>
    </QueryParam>    
</ExtractVariables>
```
...
```
/payments/d4908629-7ae5-11e7-ba80-0ad881f403bf
```

![image alt text](./media/image_7.png)

* The above request returns a specific record from the payments collection

* Now modify the request to return another specific record

```
/payments/d4908629-7ae5-11e7-ba80-0ad881f403bg
```
...
* *Congratulations!*  You’ve found the problem.  Your target service cannot find an entity with the ID provided (d4908629-7ae5-11e7-ba80-0ad881f403bg).  This is a trivial example, but you can see how the tool -- providing before and after insight into message, query, and header contents -- would be of immense use in diagnosing malformed requests and other common issues.


* Now modify the request to return any specific record with a specific payment amount
* Edit your URL field once more to follow the pattern below

```
/payments?amount=89.61
```
...
* Expand the filters pane on the left side of your screen.  Then add a query parameter filter named ‘payment’.  Put the amount in the value column and ensure the URL also holds the amount, like before.

```
/payments?amount={our amount}
```
...
* Update the URL with an amount, an value for the initials query parameter.  Example below.

```
/payments?amount=89.64
```
